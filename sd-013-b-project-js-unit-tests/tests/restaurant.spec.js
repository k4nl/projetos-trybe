...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const objetoRetornado = createMenu(menuCadastrado); 
    let menuDoRestaurante = {};
    let pedidoZerado = objetoRetornado.consumption;
    ...
    assert.deepStrictEqual(createMenu(menuDoRestaurante).fetchMenu(), menuDoRestaurante);
    ...
    // objetoRetornado.consumption // Retorno: ["coxinha"] 10
    ...
    // ```
    menuDoRestaurante = createMenu();
    menuDoRestaurante.order('coxinha');
    menuDoRestaurante.order('agua');
    menuDoRestaurante.order('coxinha');
     // Retorno: ['coxinha', 'agua', 'coxinha']
    assert.deepStrictEqual(menuDoRestaurante.consumption, ['coxinha', 'agua', 'coxinha']);
    // Agora faça o TESTE 8 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
    // ```
    novoMenu = createMenu(menuCadastrado);
    novoMenu.order('coxinha');
    novoMenu.order('agua');
    novoMenu.order('coxinha');
    
    // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos
    assert.deepStrictEqual(novoMenu.pay(), 12.87)
